<script lang="ts">
	...
	function drawLabel(username: string, healthPct: number, flashType: 'heal' | 'damage' | null, flashIntensity: number, buffTypes: string[]): void {
		...
		// Username text - white for other players
	...
	useTask((delta) => {
		...
		const buffTypes = player.activeBuffs.map(b => b.type);
		...
		const needsRedraw = player.username !== lastUsername || buffKey !== lastBuffKey || healthAnimating || flashing;

		if (needsRedraw) {
			lastUsername = player.username;
			lastBuffKey = buffKey;

			drawLabel(player.username, displayedPct, healthFlashType, healthFlashTimer, buffTypes);
